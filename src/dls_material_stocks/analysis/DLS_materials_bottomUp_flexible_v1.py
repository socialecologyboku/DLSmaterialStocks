...
## define main paths which are not being modified for different modelling cases
country_correspondence_filename = "country_correspondence.xlsx"

# thresholds per capita
# assumption that 1.6 m²/cap // 10 m²/pupil for good health care and education system (Veléz-Henao & Pauliuk 2023)
m2_cap_health = 1.6
m2_pupil_education = 10

def calc_bottomUp_DLSS(
    input_paths_dict,
    output_paths_dict,
    sensitivity_factors,
    ...
    # country corespondence MISO2 (Wiedernhofer et al.,2024) & DLS data (Kikstra et al., 2025 updated)
    ...
    global_average_thresholds = pd.read_excel(change_DLS_thresholds)
    ...
    """####################
        FOOD: DIRECT FLOWS 
       ####################"""
    # note! 2019 values at scale are currently calculated with 2015 population figures - not corrected yet as we don't use 2019'''
    ...
    food_mappings = pd.read_excel(input_paths_dict.get("food_map"))
    ...
    nutrition_indstocks_dict = read_excel_into_dict(input_paths_dict.get("Velez_nutrition"))
    ...
    communic_indstocks_dict = read_excel_into_dict(input_paths_dict.get("Velez_communic"))
    ...
    housing_archetypes = pd.read_excel(input_paths_dict.get("housing_archetypes_path"))
    ...
    housing_mapping = pd.read_excel(input_paths_dict.get("country_correspondence"), sheet_name="MISO_SSP")
    ...
    housing_mat_intens_mapped["stock type"] = housing_mat_intens_mapped["stock type"].replace(
        {"RS": "SFH", "RM": "MFH"})
    ...
    housing_direct_prov_cap = housing_direct_prov_cap.iloc[:, 19:]
    ...
    housing_direct_cap_thresh = housing_direct_cap_thresh.iloc[:, 19:]
    ...
    shelter_indstocks_dict = read_excel_into_dict(input_paths_dict.get("Velez_shelter"))
    ...
    for key, value in country_correspondence_dict.items():
        modal_shares["region"] = modal_shares["region"].replace({key: value})
    ...
    mobility_indstocks_dict = read_excel_into_dict(input_paths_dict.get("Velez_mobility"))
    ...
    occupancy_rate = pd.read_excel(occupancy_data_path
    ).rename(
        columns={
            "R11.region": "region",
    ...
    R11_country_correspondence = pd.read_excel(input_paths_dict.get("country_correspondence")
    )
    ...
    clothing_indstocks_dict = read_excel_into_dict(input_paths_dict.get("Velez_hygiene_clothing"))
    ...
    # SANITATION FLOWS
    sanitation_needs = pd.read_excel(input_paths_dict.get("MISO2_sanitation"), sheet_name="sanitation")
    ...
    sanitation_water_indstocks_dict = read_excel_into_dict(
        input_paths_dict.get("Velez_hygiene_water")
    )
    ...
    # WATER FLOWS
    water_needs = pd.read_excel(input_paths_dict.get("MISO2_sanitation"), sheet_name="water")
    ...
    educ_shares = pd.read_csv(input_paths_dict.get("education_shares_path")).rename(
        columns={"iso": "region"}
    )
    # rename regions according to MISO2 country labels and drop all which are not in MISO2 country list
    for key, value in country_correspondence_dict.items():
        educ_shares["region"] = educ_shares["region"].replace({key: value})
    ...
    # 3) map material intensities to achievement and calculate direct stocks
    buildings_mapping = housing_mapping.copy().rename(
        columns={"region": "Haberl_et_al", "MISO2_country": "region"}
    )
    ...
    educ_indirect_stocks_thresh.sum() / 1e9
    ...
    for i in [energy_prov_cap, energy_thresh_cap]:
        for key, value in country_correspondence_dict.items():
            i["region"] = i["region"].replace({key: value})
    ...
    fuel_prov_cap_op = fuel_prov_cap[["current.dle.op", "oil", "coal", "gas"]].copy()
    fuel_prov_cap_con = fuel_prov_cap[["current.dle.conrep", "oil", "coal", "gas"]].copy()
    fuel_thresh_cap_op = fuel_thresh_cap[["thres.energy.op", "oil", "coal", "gas"]].copy()
    fuel_thresh_cap_con = fuel_thresh_cap[["thres.energy.conrep", "oil", "coal", "gas"]].copy()
